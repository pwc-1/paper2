...
class VideoDataset():
    ...
    def __getitem__(self, index):
        ...
        match_score_start, match_score_end, match_score_action, gt_iou_map, num_action = self._get_train_label(index, self.anchor_xmin,
                                                                                         self.anchor_xmax)
        if self.subset == "train":
            return video_data, match_score_start, match_score_end, match_score_action, num_action
        return video_data, num_action
    ...
    def _get_train_label(self, index, anchor_xmin, anchor_xmax):
        ...
        #############################
        # num_action = np.array([len(video_labels)])
        if len(video_labels) == 1:
            num_action = [1]
        else:
            num_action = [0]
        num_action = np.array(num_action)
        ##############################################################################################
        ...
        match_score_action = np.array(match_score_action)

        return match_score_start, match_score_end, match_score_action, gt_iou_map, num_action

    def __len__(self):
        return len(self.video_list)

def createDataset(cfg, mode) -> Tuple[ds.BatchDataset, dict]:
    """create AN_classifier dataset"""
    ...
    if mode == 'train':
        columns = ['features', 'confidence_score', 'match_score_start', 'match_score_end', 'num_action']
    else:
        columns = ['features', 'num_action']
    shuffle = mode == "train"
