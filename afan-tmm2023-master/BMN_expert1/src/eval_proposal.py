...
class ANETproposal():
    ...
    def __init__(self, ground_truth_filename=None, proposal_filename=None,
        ...
        anno_df = pd.read_csv("/opt/data/private/yptang/datasets/TAL/Anet/activitynet_annotations/video_info_action.csv")
        self.df_list = []
        for i in range(len(anno_df)):
            video_name =anno_df.video.values[i]
            video_n_action =anno_df.num_action.values[i]
            if video_n_action not in [1]:
               continue
            self.df_list.append(video_name)
    ...
    def _import_ground_truth(self, ground_truth_filename):
        ...
        for videoid, v in data['database'].items():
            if self.subset != v['subset']:
                continue
            vdieoid_name = 'v_' + videoid
            if vdieoid_name not in self.df_list:
                continue
