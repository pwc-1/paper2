...
def distill_loss(output_s, output_t, T):
    ceriterion = ops.KLDivLoss(reduction='mean')
    loss = ceriterion(ops.log_softmax(output_s/T, axis = 0), ops.softmax(output_t/T, axis = 0 ) * T * T)
    return loss

class AFAN_Loss(nn.Cell):
    def __init__(self, bm_mask, mode='train'):
        super(AFAN_Loss, self).__init__()
    ...
    def construct(self, pred_bm, pred_start, pred_end, 
                        pred_bm_1, pred_start_1, pred_end_1,
                        pred_bm_2, pred_start_2, pred_end_2,  
                        gt_iou_map, gt_start, gt_end, num_a):
        #hard loss
        ...
        bmn_loss = tem_loss + 10 * pem_reg_loss + pem_cls_loss
        ...
        for i in range(bs):
            if num_a[i] == 1:
                kd_loss_m += 300*distill_loss(pred_bm_1[i].reshape(-1), pred_bm[i].reshape(-1), 1)
                kd_loss_s += 3*distill_loss(pred_start_1[i], pred_start[i], 1)
                kd_loss_e += 3*distill_loss(pred_end_1[i], pred_end[i], 1)
            else:
                kd_loss_m += 700*distill_loss(pred_bm_2[i].reshape(-1), pred_bm[i].reshape(-1), 1)
                kd_loss_s += 7*distill_loss(pred_start_2[i], pred_start[i], 1)
                kd_loss_e += 7*distill_loss(pred_end_2[i], pred_end[i], 1)
        kd_loss = 100 * (kd_loss_m + kd_loss_s + kd_loss_e)
        alpha = 0.
        # print(f'kd_loss {kd_loss} kd_loss_m {kd_loss_m} kd_loss_s {kd_loss_s} kd_loss_e {kd_loss_e}')
        print(f"kd_loss {kd_loss} bmn_loss {bmn_loss}")
        loss = alpha * kd_loss + (1 - alpha) * bmn_loss

        if self.mode == "train":
            return loss
        losses = self.stack([bmn_loss, tem_loss, pem_reg_loss, pem_cls_loss])
        return losses
