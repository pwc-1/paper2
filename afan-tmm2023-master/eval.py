...
from src.afan import AFAN, AFANWithEvalCell
from src.an_classifier import AN_classifier
from src.loss import get_mask
from src.config import config as cfg
from src.dataset import createDataset
from src.metrics import AFANMetric
...
if __name__ == '__main__':
    ...
    an_classifier = AN_classifier(cfg.model)
    ...
    param_dict = load_checkpoint("./chekpoints/AFAN_an_classifier.ckpt") #an_classifier weights
    load_param_into_net(an_classifier, param_dict)

    #net
    eval_net = AFANWithEvalCell(network, an_classifier)

    # metrics
    metric = AFANMetric(cfg, subset="validation")
    ...
    logger.info("Exp. %s - collecting eval results success", (cfg.experiment_name))
