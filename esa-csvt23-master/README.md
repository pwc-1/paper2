...
The Mindspore core code of the ESA from TCSVT 2023 paper [“ESA: External Space Attention Aggregation for Image-Text Retrieval”](https://ieeexplore.ieee.org/document/10061465) (Finished in 2021)
...
2. run ./train_xxx_f30k.sh or ./train_xxx_coco.sh. For example:
  ...
  ```
3. Evaluation: Run the following shell and change the corresponding paths. The evaluation has been re-implemented by Mindspore architecture.
  ```
  sh eval_GRU_f30k.sh
  ```
4. Evaluation: Run the following commands after modifying the default data and model path to yourself path.
  ```
  cd ../
